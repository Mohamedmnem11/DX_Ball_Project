...
Gpio_Return_t GPIO_ConfigureInterrupt(InterruptConfig_t* config) {
    ...
    volatile uint32_t* nvicPriReg = (volatile uint32_t*)(0xE000E400 + (4 * config->priRegister));
    volatile uint32_t* nvicEnReg = (volatile uint32_t*)(0xE000E100 + (4 * (config->interruptNumber / 32)));
    
    
    
    uint32_t priorityMask = 0xFF << config->priorityShift;
    *nvicPriReg = (*nvicPriReg & ~priorityMask) | 
                  ((uint32_t)config->priority << config->priorityShift);
    
    *nvicEnReg = 1UL << (config->interruptNumber % 32);
