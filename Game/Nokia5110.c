...
void Nokia5110_PrintBMP2(unsigned char xpos, unsigned char ypos, const unsigned char *ptr, unsigned char threshold, unsigned char clear) {
    ...
    for (i = 1; i <= (width * height / 2); i = i + 1) {
        if (!clear) {  // Draw the bitmap
            // Left pixel is in the upper 4 bits
            if (((ptr[j] >> 4) & 0xF) > threshold) {
                Screen[screenx] |= mask;
            } else {
                Screen[screenx] &= ~mask;
            ...
            // Right pixel is in the lower 4 bits
            if ((ptr[j] & 0xF) > threshold) {
                Screen[screenx] |= mask;
            } else {
                Screen[screenx] &= ~mask;
            ...
            Screen[screenx] &= ~mask;  // Clear the left pixel
            screenx = screenx + 1;
            Screen[screenx] &= ~mask;  // Clear the right pixel
        }
        ...
        if ((i % (width / 2)) == 0) {  // At the end of a row
            ...
            // Skip any padding (bitmaps are 32-bit word aligned)
            switch ((width / 2) % 4) {
                case 0: j = j + 0; break;
                case 1: j = j + 3; break;
                case 2: j = j + 2; break;
